    ...
    Calendar.prototype = {
        ...
        showCalendar: function() { // 输入数据并显示
            ...
            this.$calendarDate_item.each(function(i) {
                ...
                if (self.opts.days.length > 0) {
                    for (var i = 0; i < self.opts.days.length; i++) {
                        if (allDay_str == self.opts.days[i]) {
                            $(this).addClass("calendar-date-select");
                        }
                    }
                }
        ...
        renderDOM: function() { // 渲染DOM
            this.$calendar_title = $('<div class="calendar-small-title"></div>');
            this.$calendar_week = $('<ul class="calendar-small-week"></ul>');
            this.$calendar_date = $('<ul class="calendar-small-date"></ul>');
            this.$calendar_today = $('<div class="calendar-small-today"></div>');
            ...
            var _titleStr = '<a href="#" class="title"></a>' +
                ...
                '</div>' + '<a style="position: absolute;right: 0px;top: -10px;" class="timeclose">X</a>';
        ...
        inital: function() { // 初始化
            ...
            this.$close = this.$calendar_title.find('.timeclose');
